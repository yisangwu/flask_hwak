...
def index():
    ...
    """
    # 分割出module，function
    module, xfunction = request.x_method.split(MOTHOD_SEPARATE)
    if not module or not xfunction:
        return HelperRet.ret_json(rdata='请求接口%s格式错误！' % request.x_method, rcode=-200)

    # 动态加载接口模块
    try:
        interface_file = importlib.import_module(
            'frontend.%s.%s' % (module, xfunction))
    ...
    # 动态导入接口函数
    try:
        x_function = getattr(interface_file, xfunction)
    except Exception as e:
        # 找不到接口函数
